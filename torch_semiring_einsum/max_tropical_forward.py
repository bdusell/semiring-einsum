...
from .utils import max_in_place, max_block, add_in_place

def max_tropical_einsum_forward(
        equation: Equation,
        *args: torch.Tensor,
        block_size: typing.Union[int, AutomaticBlockSize]=AUTOMATIC_BLOCK_SIZE
    ) -> torch.Tensor:
    r"""Max tropical einsum, where addition (:math:`a + b`) is replaced with
    max (:math:`\max(a, b)`) and multiplication (:math:`a \times b`) is
...
def _callback(compute_sum):
    return compute_sum(max_in_place, max_block, add_in_place)
